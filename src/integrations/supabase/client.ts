...
export const createFinalInvoice = async (data: any) => {
  try {
    ...
    // Add user ID to the invoice data and use correct column names
    ...
    try {
      ...
      if (items && Array.isArray(items) && items.length > 0) {
        ...
        for (const item of items) {
          ...
          // Link the item to the final invoice
...
// Invoice payment functions - mock implementation since table doesn't exist
export const addInvoicePayment = async (invoiceId: string, paymentData: any) => {
  try {
    console.log('Mock payment added for invoice:', invoiceId, 'with data:', paymentData);
...
export const getInvoicePayments = async (invoiceId: string) => {
  try {
    console.log('Mock: fetching payments for invoice:', invoiceId);
...
export const deleteInvoicePayment = async (paymentId: string, invoiceId: string) => {
  try {
    console.log('Mock: deleting payment', paymentId, 'for invoice', invoiceId);
    return { success: true };
  } catch (error) {
